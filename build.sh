...
fullname="${account}.dkr.ecr.${region}.amazonaws.com/dhiraj-repo-inference:latest"

# If the repository doesn't exist in ECR, create it.
# The pipe trick redirects stderr to stdout and passes it /dev/null.
# It's just there to silence the error.
aws ecr describe-repositories --repository-names "dhiraj-repo-inference" > /dev/null 2>&1

# Check the error code, if it's non-zero then know we threw an error and no repo exists
if [ $? -ne 0 ]
then
    aws ecr create-repository --repository-name "dhiraj-repo-inference" > /dev/null
fi
...
docker build  -t "dhiraj-repo-inference" .
#docker tag "sagemaker-demo" ${fullname}
docker tag "dhiraj-repo-inference" ${fullname}
echo "Pushing image"
